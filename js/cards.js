...
const container = document.querySelector(".cards[data-type='cards']");
...
function createCardElement(game) {
  ...
  cardInfoElement.textContent = `${game.short_description.slice(0, 70)}...`;
  ...
  const cardPublisherElement = cardElement.querySelector(
    "[data-card-publisher]"
  );
  cardPublisherElement.textContent = `Publisher: ${game.publisher}`;

  const cardDeveloperElement = cardElement.querySelector(
    "[data-card-developer]"
  );
...
function filterAndRenderCards() {
  ...
  if (checkboxNew.checked) {
    filteredGames = games
      .slice(0, 50)
      .filter((game) => getReleaseYear(game) > 2020);
  } else if (checkboxOld.checked) {
    filteredGames = games
      .slice(0, 50)
      .filter((game) => getReleaseYear(game) < 2010);
  } else {
  ...
  if (platformRadio.checked) {
    filteredGames = filteredGames.filter((game) =>
      game.platform.toLowerCase().includes("pc (windows)")
    );
  } else if (onlineGamesRadio.checked) {
    filteredGames = filteredGames.filter((game) =>
      game.platform.toLowerCase().includes("web browser")
    );
  }
...
window.addEventListener("scroll", function () {
  let scrolledPage = window.scrollY;
  // console.log(scrolledPage);

  if (scrolledPage > 1000) {
